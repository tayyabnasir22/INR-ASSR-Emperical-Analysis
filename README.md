...
Paper: [arXiv:2601.17723](https://arxiv.org/abs/2601.17723)
...
```bibtex
@article{nasir2026implicitneuralrepresentationbenchmark,
...
| Recipe | Loss | Input Patch | Scale Range | LR Scheduler | Description |
|--------|------|-------------|-------------|--------------|-------------|
| `TrainSimple` | L1 | 48×48 | 1–4 | Multi-step | A baseline configuration for standard INR-ASSR training. |
| `TrainSimpleLargerPatch` | L1 | 64×64 | 1–4 | Multi-step | Trains with larger input patches to enhance spatial consistency. |
| `TrainSimpleLargerPatchScale` | L1 | 64×64 | 1–6 | Multi-step | Extends `TrainSimpleLargerPatch` by covering a broader range of scales. |
| `TrainSGDR` | L1 | 48×48 | 1–4 | Cosine Annealing (SGDR) | Uses cosine annealing to enable smoother learning rate transitions. |
| `TrainGradLoss` | Gradient L1 | 48×48 | 1–4 | Multi-step | Incorporates gradient-based supervision to encourage structural and texture fidelity. |
| `TrainGramL1Loss` | Gram L1 | 48×48 | 1–4 | Multi-step | Introduces Gram-based loss for texture consistency and perceptual enhancement. |
...
2. Adjust the parameters — loss function, patch size, scale range, or learning rate scheduler — to match your experimental setup.
3. Register the new configuration in `TrainingOrchestrator.py`.
...
The saving/loading system is fully configurable. You can easily modify parameters to:

- Load **specific epoch checkpoints**
- Switch between **different testing strategies** — full image, patch-based, or overlapped-patch testing — to handle **GPU memory constraints** using smaller patch sizes

The logic for constructing save paths can be customized in **`Utilities.PathManager.GetModelSavePath`**.
...
## Understanding Coordinate-Based Sampling

For an explanation of how coordinate-based sampling works in INR-based super-resolution, see the included notebook: [WorkingOfCoordinatesExplained.ipynb](WorkingOfCoordinatesExplained.ipynb).
